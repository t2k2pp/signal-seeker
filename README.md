...
> **PowerShell での注意**: PowerShell から `npm run ... -- --channel=<id>` を実行すると、`npm`(=`npm.ps1`)が
> `--channel` を自分の設定として横取りし、チャンネルが渡りません。PowerShell では次のいずれかを使ってください。
> - `npm.cmd run crawl -- --channel=ai-tech` (cmd シムを直接呼ぶ。最も簡単)
> - `npx tsx src/index.ts --channel=ai-tech` (スクリプトを直接実行)
>
> Bash / cmd.exe ではそのまま `npm run crawl -- --channel=ai-tech` で動きます。

定期実行は OS のスケジューラに委譲してください(**チャンネルごとにタスクを登録**します)。

- **Windows (タスクスケジューラ)**: 「プログラム」に `npm.cmd`(PowerShell 経由を避けるため `.cmd`)、
  「引数」に `run crawl -- --channel=ai-tech`、「開始(作業フォルダ)」に本リポジトリのパスを指定。
...
収集した内容を**ブラウザで横断的に閲覧**できるローカル Web 画面です。収集・要約・通知(CLI)はそのままに、
「今どのチャンネルに何があるか」を 1 か所で見られます。**全チャンネル横断の俯瞰・トリアージ / 全文検索 /
過去レポート・実行履歴の閲覧 / ソース設定の管理**ができます。

```bash
# 1) 初回だけフロントエンドを用意(web/ ディレクトリ)
cd web && npm install && npm run build && cd ..

# 2) サーバを起動(http://127.0.0.1:8787 をブラウザで開く)
npm run serve
```

- アクセスは **自分のPC内(127.0.0.1)に限定**しています。外部には公開しません。
- データベースは**読み取り専用**で開くため、`npm run crawl` の最中でも安全に閲覧できます。
- 画面からの変更は**ソース設定(有効/無効・追加・削除)だけ**で、`config/channels/<id>/sources.json` に保存され、
  **次回の収集から反映**されます。記事データそのものは画面からは変更しません。
- 検索は3文字以上で全文検索が効きます(2文字以下は部分一致検索)。
- ポートを変えるには `SIGNALSEEKER_PORT=9000 npm run serve`(または `npm run serve -- --port=9000`)。
- 画面を改造したいときは、開発中は `cd web && npm run dev`(`/api` は起動中の `npm run serve` へ自動中継)。

> PowerShell の場合: 上の `npm run serve` はそのまま動きます(`--channel` 等の引数を渡すコマンドだけ `npm.cmd` か
> `npx tsx` を使う注意が残ります。「実行」節を参照)。
