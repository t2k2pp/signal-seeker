...
// - 既定は終端から過去7日。--days は 1..15(最大15日。月次は別軸で対応予定)。
// 生成物: data/<channel>/reports/report-weekly-<end>-<days>d.md と .json(スナップショット)。
// HTML化は別途 `npm run infographic -- --channel=<id> --label=weekly-<end>-<days>d` を手動実行する。
...
/** 1チャンネル分の週次レポートを生成する。 */
function runWeekly(channel: ResolvedChannel, days: number, until: string | undefined): void {
  const store = new SqliteStore(channel.paths.db);
  try {
    const all = store.allSummarizedItems();
    if (all.length === 0) {
      console.log(`[${channel.name}] 要約済みのデータがありません(先に npm run crawl -- --channel=${channel.id})。`);
      return;
    }
    ...
    const summarized: SummarizedItem[] = all.filter(inWindow).map((it) => ({
      ...
      state: "new", // 週次では状態バッジを出さない。型を満たすための値。
    }));

    const now = new Date().toISOString();
    const result: RunResult = { runId: null, startedAt: now, finishedAt: now, summarized, errors: [] };

    const model = buildReportModel(result, channel.config.curation, {
      runId: null,
      date: end,
      kind: "weekly",
      period,
    ...
    const mdPath = notifyConsole(markdown, label, channel.paths.reportsDir);
    writeSnapshot(result, label, channel.paths.reportsDir, { kind: "weekly", period });

    console.log(`\n[${channel.name}] 週次レポート生成(${start} 〜 ${end}・${days}日間・${summarized.length}件)`);
    console.log(`  Markdown: ${mdPath}`);
    console.log(`  HTML化(任意): npm run infographic -- --channel=${channel.id} --label=${label}`);
  } finally {
    store.close();
  }
}
...
for (const id of channelsOrExit()) {
  runWeekly(resolveChannel(id), days, until);
}
