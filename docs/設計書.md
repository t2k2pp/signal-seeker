...
#### 可視化レイヤ(`web/src/viz.tsx`)

「データを読ませる」だけでなく「シグナルの発見を魅せる」ための表示部品群。外部チャート依存を増やさず
**純 SVG + CSS アニメ**で実装する(server を `node:http` のみで組んだ軽量方針に合わせる)。

- **CountUp**: 概要の全体サマリ(総シグナル/要約済み/未配信/稼働チャンネル)を 0 から目標値へ
  ease-out cubic でカウントアップ。
- **Sparkline**: `/api/channels` が返す `activity`(直近14回の収集ごとの新着件数、古い→新しい)を細い折れ線で描画。
- **LiveDot + `relTime`**: `/api/channels` の `lastRun`(最終収集の時刻・状態・新着件数)から「最終収集 3時間前 ●」を表示。
  状態 `completed`/`running` は緑(脈打つ)、`failed` は赤(静止)。
- **ScoreBar**: スコアを「データ内の最大スコアに対する相対比」で横ゲージ化。比率 0.8 以上は accent 色で発光(`hot`)。
  概要のヒーロー/ランキング、チャンネル一覧、記事行で使用。
- **Rank**: 横断トリアージの順位バッジ(1〜3位は金銀銅)。
- **AttentionChips**: 注目度(HF upvotes/引用/GitHub reactions/stars)を、種類ごとの対数基準で満たした
  ミニ棒(背景グラデーションの充填率)で表示。種類間で目盛りを共有しない(桁が違うため)。

`/api/channels` はこの可視化のために `activity: number[]` と `lastRun: {at,status,newCount} | null` を追加で返す
(`recentRuns(14)` から導出。webhook 等の秘密は従来どおり一切返さない)。スコアの正規化基準(`max`)は
取得済みデータ内の最大値で、追加の集計 API は不要。
