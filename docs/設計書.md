...
```mermaid
flowchart TD
    SRC["config/sources.json（訪問先リスト＝取得情報を変える唯一のレバー）"] --> COL
    subgraph crawl["npm run crawl（収集→要約→配布→Wiki）"]
        COL["① Collect：rss-parser / Playwright・本文取得・差分判定"] --> DB[("SQLite：data/signalseeker.db<br/>要約も含め永続化・reported で配信管理")]
        DB --> SUM["② Summarize：生成AI（Claude / ローカルLLM）"]
        SUM --> DB
        DB --> REP["③ Report / Notify"]
        REP --> MD["data/reports/report-*.md"]
        REP --> SNAP["data/reports/report-*.json（スナップショット）"]
        REP --> DISC1(["Discord webhook（md添付）"])
        DB --> WIKI["Wiki：data/wiki/（Obsidian vault）"]
    end
    SNAP --> IG["npm run infographic（整形・任意・手動）"]
    IG --> HTML["data/reports/report-*.html"]
    IG --> DISC2(["Discord webhook（html添付）"])
    SUM -.-> LOG["data/logs/（実行JSONL + LLM IO全文）"]
```
...
- **Markdown**: `# SignalSeeker レポート <date> (run #<id>)` → **カテゴリ別件数の Mermaid 円グラフ**
  ...
  同一リリース系列は代表+`<details>`折りたたみ。Mermaid は GitHub/Obsidian/VSCode 等でレンダリングされる。
...
- `npm run infographic -- [--run=<id> | --label=<label> | --latest] [--no-discord]`(既定 `--latest`)。
  出力 `data/reports/report-<runLabel>.html`。
- **手動実行でも Discord へ送付**する: `config.notify.targets` に `discord` があり `DISCORD_WEBHOOK_URL` が
  設定されていれば、生成した HTML を添付して送る(本文は run-id と件数の短文)。`--no-discord` で抑止。
